...
def store_callback(context: dict, message: dict):
    ...
    # Transaction status changes are driven by the action, not the contract payload.
    # A confirm with an empty contract is still a confirm.
    if action == "on_confirm":
        txn["status"] = "CONFIRMED"
    elif action == "on_status":
        txn["status"] = "COMPLETED"

    # Accumulate contract fields only when contract data is present
