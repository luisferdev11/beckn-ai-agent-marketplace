#!/usr/bin/env python3
"""
BPP Conformance Test Kit — Beckn AI Agent Marketplace.
...
What we DO test (directly against your BPP's HTTP surface):
    - /health responds < 500 ms
    - Beckn webhooks ACK valid envelopes
    - select rejects unknown agents with error.code = "30001"
    - init/confirm/status/cancel reject unknown transactions with "30002"
    - on_publish webhook ACKs the callback shape
    - Malformed requests don't crash with 500
    - If you expose /api/catalog, items validate against AgentFacts v1

What we do NOT test (out of scope for this kit):
    - Signature verification (requires ONIX in the loop)
    - The full publish→index→on_publish round trip
    - Multi-step transaction lifecycle persistence

For a full integration smoke against the marketplace, the operator runs
scripts/smoke_test_dual_bpp.py with your BPP wired in.
...
async def t_init_unknown_txn(ctx: TestContext) -> TestResult:
    ...
    # ACK is fine sync; the rejection should arrive in the on_init callback
    # which we can't observe from here. Many BPPs also surface it inline.
    if status >= 500:
    ...
    # If the BPP returns the error inline, check for 30002. Otherwise pass on the
    # weaker check that the BPP at least didn't crash.
...
async def t_catalog_shape(ctx: TestContext) -> TestResult:
    ...
    # Two common shapes: {"resources": [...]} (Tecla) or {"catalogs":[{"resources":[...]}]} (Beckn shape)
...
async def run(ctx: TestContext) -> tuple[int, list[TestResult]]:
